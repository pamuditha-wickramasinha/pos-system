			...
			if (result[0]) {
				...
				if (result[0] == "success") {
						...
						if (!print) {
							...
							// No printer configured on this device yet - fall back to the old
							// print-preview popup so the cashier can still print manually.
...
function hold_invoice_list() {
	...
	$.post(base_url + "pos/hold_invoice_list", {}, function (result) {
		//alert(result);
		var data = parse_ajax_json(result)
