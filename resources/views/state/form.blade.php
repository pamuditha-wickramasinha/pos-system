@extends('layouts.app')
@php($activeMenu = 'state')

@section('content')
<section class="content-header">
    <h1>{{ $page_title }} <small>Add/Update State</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ url('dashboard') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><a href="{{ route('state.index') }}">States List</a></li>
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="box box-info">
                <form class="form-horizontal" id="state-form" onkeypress="return event.keyCode != 13;">
                    @csrf
                    <input type="hidden" id="base_url" value="{{ url('/').'/' }}">
                    <div class="box-body">
                        <div class="form-group">
                            <label for="country" class="col-sm-2 control-label">Country<label class="text-danger">*</label></label>
                            <div class="col-sm-4">
                                <select class="form-control select2" id="country" name="country" style="width: 100%;">
                                    ...
                                    @foreach (\App\Models\Country::where('status', true)->get() as $c)
                                        <option @selected(($country ?? '') === $c->country) value="{{ $c->country }}">{{ $c->country }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group">
                            <label for="state" class="col-sm-2 control-label">State<label class="text-danger">*</label></label>
                            <div class="col-sm-4">
                                <input type="text" class="form-control input-sm" id="state" name="state" value="{{ $state ?? '' }}" onkeyup="shift_cursor(event,'save')">
