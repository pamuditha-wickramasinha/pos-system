@extends('layouts.app')
@php($activeMenu = 'sales-return')
@php($customerId = $returnEntry?->customer_id ?? $sale?->customer_id)
@php($customerName = $customerId ? \App\Models\Customer::find($customerId)?->customer_name : null)
@php($saveOperation = $oper !== 'edit_existing_return')
...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{ route('sales_return.index') }}">Sales Return List</a></li>
        <li><a href="{{ route('sales_return.create') }}">New Sales</a></li>
                ...
                <form class="form-horizontal" id="sales-form" enctype="multipart/form-data">
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            @if($sale)
                                <label class="col-sm-2 control-label">Sales Code<label class="text-danger">*</label></label>
                                <label class="col-sm-3 control-label" style="text-align: left;">{{ $sale->sales_code }}</label>
                            @endif
                            @if($customerName)
                                <label class="col-sm-2 control-label">Customer Name<label class="text-danger">*</label></label>
                                <label class="col-sm-3 control-label" style="text-align: left;">{{ $customerName }}</label>
                    ...
                    <div class="box-footer col-sm-12">
                        <center>
                            @if($oper === 'return_against_sales')
                                ...
                                <input type="hidden" name="sales_id" id="sales_id" value="{{ $sale->id }}">
                            @elseif($oper === 'edit_existing_return')
                                ...
                                <input type="hidden" name="sales_id" id="sales_id" value="{{ $returnEntry->sales_id }}">
                            @else
...
function save_operation() { return {{ $saveOperation ? 'true' : 'false' }}; }
...
function calculate_tax(i){
    ...
    var amt = parseFloat(qty) * sales_price;
