@extends('layouts.app')
@php($activeMenu = 'purchase')
@php($purchaseId = $purchase?->id ?? null)

@section('content')
<section class="content-header">
    <h1>{{ $page_title }} <small>Add/Update Purchase</small></h1>
    <ol class="breadcrumb">
                ...
                <form class="form-horizontal" id="purchase-form" enctype="multipart/form-data">
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <label for="pur_date" class="col-sm-2 control-label">Purchase Date <label class="text-danger">*</label></label>
                            <div class="col-sm-3">
                                <div class="input-group date">
                                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                    <input type="text" class="form-control pull-right datepicker" id="pur_date" name="pur_date" readonly value="{{ $purchase?->purchase_date ? show_date($purchase?->purchase_date) : show_date(date('d-m-Y')) }}">
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="purchase_status" class="col-sm-2 control-label">Status <label class="text-danger">*</label></label>
                            <div class="col-sm-3">
                                <select class="form-control select2" id="purchase_status" name="purchase_status" style="width: 100%;">
                                    <option @selected(($purchase?->purchase_status ?? '') === 'Received') value="Received">Received</option>
                                    <option @selected(($purchase?->purchase_status ?? '') === 'Pending') value="Pending">Pending</option>
                                    <option @selected(($purchase?->purchase_status ?? '') === 'Ordered') value="Ordered">Ordered</option>
                                </select>
                            </div>
                            <label for="reference_no" class="col-sm-2 control-label">Reference No</label>
                            <div class="col-sm-3">
                                <input type="text" value="{{ $purchase?->reference_no ?? '' }}" class="form-control" id="reference_no" name="reference_no">
                            </div>
                        </div>
                    </div>
                                            ...
                                            <div class="table-responsive" style="width: 100%">
                                                <table class="table table-hover table-bordered" style="width:100%" id="purchase_table">
                                                    ...
                                                    <tr class="bg-primary">
                                                        ...
                                                        <th rowspan="2" style="width:15%">Purchase Price({{ $CURRENCY }})</th>
                                                        ...
                                                        <th rowspan="2" style="width:7.5%">Unit Cost({{ $CURRENCY }})</th>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-4">
                                            <input type="text" class="form-control text-right only_currency" id="other_charges_input" name="other_charges_input" onkeyup="final_total();" value="{{ $purchase?->other_charges_input ?? '' }}">
                                        </div>
                                        <div class="col-sm-4">
                                            <select class="form-control" id="other_charges_tax_id" name="other_charges_tax_id" onchange="final_total();" style="width: 100%;">
                                                ...
                                                @foreach (\App\Models\Tax::where('status', true)->get() as $t)
                                                    <option @selected(($purchase?->other_charges_tax_id ?? null) == $t->id) data-tax="{{ $t->tax }}" value="{{ $t->id }}">{{ $t->tax_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-4">
                                            <input type="text" class="form-control text-right only_currency" id="discount_to_all_input" name="discount_to_all_input" onkeyup="enable_or_disable_item_discount();" value="{{ $purchase?->discount_to_all_input ?? '' }}">
                                        </div>
                                        <div class="col-sm-4">
                                        ...
                                        @if(! empty($purchase?->discount_to_all_type ?? ''))
                                            document.getElementById('discount_to_all_type').value='{{ $purchase?->discount_to_all_type }}';
                                        @endif
                                        </script>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-8">
                                            <textarea class="form-control text-left" id="purchase_note" name="purchase_note">{{ $purchase?->purchase_note ?? '' }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
$(document).ready(function () {
    var supplier_id = "{{ $purchase?->supplier_id ?? '' }}";
